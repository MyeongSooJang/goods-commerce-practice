    ...
    public void publish(UUID auctionId, UUID sellerId, String auctionTitle, BigDecimal finalPrice) {
        try {
            EventEnvelope<AuctionClosedSoldPayload> envelope = new EventEnvelope<>(
                    UUID.randomUUID(),
                    AuctionEventTypes.AUCTION_CLOSED_SOLD,
                    "auction-service",
                    auctionId,
                    sellerId,
                    Instant.now(),
            ...
            kafkaTemplate.send(KafkaTopics.AUCTION_CLOSED, auctionId.toString(), json)
                    .whenComplete((result, ex) -> {
                        if (ex != null) {
                            log.error("auction-closed-sold 이벤트 발행 실패: auctionId={}, sellerId={}", auctionId, sellerId, ex);
                        } else {
                            log.debug("auction-closed-sold 이벤트 발행 성공: auctionId={}, sellerId={}", auctionId, sellerId);
                        }
                    });
